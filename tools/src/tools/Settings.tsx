    ...
    <section className="space-y-6">
      ...
      <div className="rounded-md border bg-white p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
          <label className="block">
            ...
            <input
              ...
              type="number"
              step="0.01"
              value={settings.timberPerM}
              onChange={(e) => setSettings({ timberPerM: parseFloat(e.target.value) || 0 })}
            />
          </label>
          <label className="block">
            <span>Piles ($/each)</span>
            <input
              className="mt-1 w-full rounded-md border p-2"
              type="number"
              step="0.01"
              value={settings.pilePerEach}
              onChange={(e) => setSettings({ pilePerEach: parseFloat(e.target.value) || 0 })}
            />
          </label>
          <label className="block">
            <span>Paint ($/m²)</span>
            <input
              ...
              type="number"
              step="0.01"
              value={settings.paintPerM2}
              onChange={(e) => setSettings({ paintPerM2: parseFloat(e.target.value) || 0 })}
            />
          </label>
          <label className="block">
            <span>Window ($/m²)</span>
            <input
              ...
              type="number"
              step="0.01"
              value={settings.windowPerM2}
              onChange={(e) => setSettings({ windowPerM2: parseFloat(e.target.value) || 0 })}
            />
          </label>
          <label className="block">
            <span>Door ($/unit)</span>
            <input
              ...
              type="number"
              step="0.01"
              value={settings.doorPerUnit}
              onChange={(e) => setSettings({ doorPerUnit: parseFloat(e.target.value) || 0 })}
            />
          </label>
        ...
        <p className="mt-2 text-xs text-slate-500">
          Settings save locally and sync to server when online.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
          <label className="block">
            ...
            <input
              ...
              onChange={(e) =>
                setSettings({ gstRate: Math.max(0, parseFloat(e.target.value) || 0) / 100 })
              }
            />
          </label>
        </div>
